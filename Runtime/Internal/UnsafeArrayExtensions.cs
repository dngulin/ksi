...
namespace Ksi
{
    ...
    {
        public static unsafe void Clear<[ExplicitCopy, DynSized, TempAlloc] T>(this ref UnsafeArray<T> self, int len)
        ...
        public static unsafe ref T Index<[ExplicitCopy, DynSized, Dealloc, TempAlloc] T>(this in UnsafeArray<T> self, int idx)
            where T : unmanaged => ref self.Buffer[idx];

        public static unsafe void Resize<[ExplicitCopy, DynSized, TempAlloc] T>(this ref UnsafeArray<T> self, int len, Allocator allocator) where T : unmanaged
        {
            if (self.Length == len)
                return;
